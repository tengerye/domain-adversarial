...
tf.app.flags.DEFINE_integer('batch_size', 25, #200, #25
                            """Number of images to process in a batch.""")
tf.app.flags.DEFINE_string('data_dir', './cifar10_data',
                           """Path to the CIFAR-10 data directory.""")
tf.app.flags.DEFINE_integer('ada_factor', 0.5,
                            """adaptation factor""")
...
INITIAL_LEARNING_RATE = 0.1#0.000000001       # Initial learning rate.
...
def inference(images):
  ...
  with tf.variable_scope('local5') as scope:
    #conv3_grl = tf.mul(conv3, -1*FLAGS.ada_factor)
    # Move everything into depth so we can perform a single matrix multiply.
  ...
  with tf.variable_scope('local6') as scope:
    weights = _variable_with_weight_decay('weights', shape=[192, 192],
                                          stddev=0.04, wd=0.004)
    biases = _variable_on_cpu('biases', [192], tf.constant_initializer(0.1))
  ...
  with tf.variable_scope('logistic') as scope:
    ...
    logistic = tf.add(tf.matmul(local6, weights), biases, name=scope.name)
    _activation_summary(logistic)
  ...
  return softmax_linear, logistic#norm1, norm2, conv3, softmax_linear, logistic
...
def loss_without_decay(logits, labels):
  """Add L2Loss to all the trainable variables.
